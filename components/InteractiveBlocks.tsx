        ...
        <div className="space-y-8 mb-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-[#CD853F]/30 before:to-transparent">
          {block.timelineItems?.map((item, idx) => (
            <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white bg-[#8B4513] text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                <i className="fas fa-calendar-alt text-xs"></i>
              </div>
              <div className="w-[calc(100%-4rem)] md:w-[45%] p-4 rounded-xl border border-gray-100 bg-white shadow-sm">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-[#8B4513]">{item.year}</div>
                </div>
                <div className="text-[#A0522D] font-semibold mb-1">{item.title}</div>
